...
import { generateRandomOpponents } from '../data/chickenModel';
import { DEFAULT_GAME_MODE } from '../data/gameModeDefs';

const titleWords = ['PRE', 'MATCH'];

export default function PreMatchScreen({ gameMode = DEFAULT_GAME_MODE, onBack, onConfirm }) {
  ...
  useEffect(() => {
    ...
    setSelectedChickenIds(seededChickens.slice(0, requiredCount).map((chicken) => chicken.id));
  }, [requiredCount]);
    ...
    >
      ...
      <div className="flex flex-wrap justify-center gap-[0.7rem]">
        ...
        <UiButton
          ...
          onClick={() =>
            onConfirm({
              playerChickens: selectedChickens,
              opponentChickens: generateRandomOpponents(requiredCount),
              gameMode,
            })
          }
          disabled={!canStart}
